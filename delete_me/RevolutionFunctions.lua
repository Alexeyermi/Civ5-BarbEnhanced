...
function UpdateSeparatist(playerID)
	local t1 = os.clock()
	local bDebug = true
...
function ReserveCS()
	...
	if #inactiveCS >=  RESERVED_CITY_STATES then
		...
		for i, playerID in ipairs(inactiveCS) do
...
function UpdateCultureRelations(playerID)
	...
	local cultureGroups = GetCultureGroups(playerID)

	for i, data in pairs(cultureGroups) do
		local relationChange = GetRelationChange(playerID, data.Type)
		Dprint ("   - relations change with " .. tostring(data.Type) .. " = " .. tostring(relationChange), bDebug)
		
		if cultureRelations[playerID][data.Type] then
			cultureRelations[playerID][data.Type] = cultureRelations[playerID][data.Type] + relationChange
		else
			cultureRelations[playerID][data.Type] = relationChange
		end

		local maximumCultureRelation = GetMaximumRelationValue(playerID)
		if cultureRelations[playerID][data.Type] > maximumCultureRelation then cultureRelations[playerID][data.Type] = maximumCultureRelation end
		if cultureRelations[playerID][data.Type] < MIN_CULTURE_RELATION then cultureRelations[playerID][data.Type] = MIN_CULTURE_RELATION end
...
function GetCultureGroups(playerID, bDebug)
...
function UpdateRebels(playerID)
	...
	local cultureMap = MapModData.AH.CultureMap
	local cultureRelations = LoadData("CultureRelations")
	if not (cultureRelations[playerID]) then
		...
		for i = 1, #civsCulture do
			if (i <= MAX_REBELS_GROUPS) and (civsCulture[i].Value > MIN_RATIO_REBELS_SPAWN) then
				...
				if cultureTypeValue >= MIN_CULTURE_REBELS_SPAWN then 
					...
					if not (cultureRelations[playerID][cultureType]) then
						Dprint ("WARNING : No entry in culture relation table for " .. cultureType)
					else
						local relation = cultureRelations[playerID][cultureType]
						if relation < MIN_RELATION_BEFORE_REBELLION then
							...
							if (rebellion > 0) and (citySize >= MIN_REBELLION_CITY_SIZE) and (rebellionChance > REBELLION_ABORT_CHANCE) then
								...
								if numRebelsToSpawn >= 1 then
									...
									local unitType = SpawnRebels( city, playerID, cultureType, numRebels) -- spawn numRebels, get type of unit spawned
							...
							if (revolution > 0) and (citySize >= MIN_REVOLUTION_CITY_SIZE) and (revolutionChance > REVOLUTION_ABORT_CHANCE) then
								...
								local revolutionResult = RevolutionInCity( city, playerID, cultureType)
...
function GetCityRebellion(city, cultureMap, cultureRelations, bDebug)
	...
	for i = 1, #civsCulture do
		...
		if (i <= MAX_REBELS_GROUPS) and (civsCulture[i].Value > MIN_RATIO_REBELS_SPAWN) then
			...
			local culturePercentValue = civsCulture[i].Value
			if not (cultureRelations[playerID]) then
				Dprint ("ERROR : cultureRelations is empty for player " .. tostring(playerID))
			elseif not (cultureRelations[playerID][cultureType]) then
				Dprint ("WARNING : No entry in culture relation table for " .. cultureType)
			else
				local relation = cultureRelations[playerID][cultureType]
			...
			local culturePercentValue = civsCulture[i].Value
			if cultureRelations[playerID][cultureType] then
				local relation = cultureRelations[playerID][cultureType]
				if relation < MIN_RELATION_BEFORE_REBELLION then
					from.Relation = relation
				end
...
function SpawnRebels( city, playerID, cultureType, numRebels)
	local bDebug = true
	...
	for n, plot in pairs(freePlots) do
		if n <= numRebels then
			if rebel:GetNumMilitaryUnits() < GetMaxRebelUnits(playerID, cultureType) then
...
function RevolutionInCity( city, playerID, cultureType)
	local bDebug = true
	local player = Players[playerID]
...
function GetGlobalHappiness(playerID, cultureRelations, bDebug)

	local bDebug = bDebug or false

	Dprint ("   - Getting Global happiness for playerID = " .. playerID, bDebug)

	local globalHappiness = 0
	local globalVariation = 0

	local cultureGroups, totalCulture = GetCultureGroups(playerID)

	if cultureRelations[playerID] then -- there could be no entries in first turns.

		for i, data in pairs(cultureGroups) do
			if cultureRelations[playerID][data.Type] then -- there could be no entries in first turns.
				local relation = cultureRelations[playerID][data.Type]
				local variation = GetRelationChange(playerID, data.Type)
				local maximumRelation = GetMaximumRelationValue(playerID)
...
function GetRelationChange(playerID, cultureType, bDebug)
	
	local bDebug = bDebug or false

	Dprint ("- Get Relation change for playerID = " .. playerID, bDebug)
	...
	data.player = Players[playerID]
	data.teamID = data.player:GetTeam()
	data.playerCultureID = GetCivTypeFromPlayer(playerID)
...
g_RelationChange = {
	...
	["TXT_KEY_RELATION_CHANGE_FROM_DOF"] = {	
		...
		Condition = (function(data)
			if not (data.cultureType == SEPARATIST_TYPE or data.cultureType == data.playerCultureID)
			   and data.otherPlayer:IsAlive() then
				if data.otherPlayer:IsDoF(data.playerID) then
					return true
				elseif data.otherPlayer:IsMinorCiv() and (data.otherPlayer:GetMinorCivFriendshipWithMajor(data.playerID) > GameDefines["FRIENDSHIP_THRESHOLD_NEUTRAL"]) then
					return true
				end
			end
			return false
	...
	["TXT_KEY_RELATION_CHANGE_FROM_DENOUNCED"] = {	
		...
		Condition = (function(data)
			if not (data.cultureType == SEPARATIST_TYPE or data.cultureType == data.playerCultureID)
			   and data.otherPlayer:IsAlive() then
				if data.otherPlayer:IsDenouncedPlayer(data.playerID) then
					return true
				elseif data.otherPlayer:IsMinorCiv() and (data.otherPlayer:GetMinorCivFriendshipWithMajor(data.playerID) < GameDefines["FRIENDSHIP_THRESHOLD_NEUTRAL"]) then
					return true
				end
...
function GetMaximumRelationValue(playerID)
				...
				if rebelID == data.RebelID then
					...
					for i = 1, #civsCulture do
						...
						if cultureType ~= SEPARATIST_TYPE and cultureType ~= rebelType then
							local player = Players[GetPlayerIDFromCivType (cultureType)]
					...
					if bestChoice then
						...
						local playerID = GetPlayerIDFromCivType (bestChoice)
						local player = Players[playerID]
					...
					elseif data.CountDown < -REVOLUTION_FORCE_CHOICE_COUNTDOWN then -- must make a choice now !
						...
						if closePlayerID then
							local player = Players[closePlayerID]
							local playerType = GetCivTypeFromPlayer(closePlayerID)
...
function GetMaxRebelUnits(playerID, rebelCultureType)
...
function RebelAttrition(playerID)
	local reservedCS = LoadData ("ReservedCS")
	if reservedCS[playerID] and reservedCS[playerID].Action == "REVOLT" then
		local player = Players[playerID]
		if not player:IsAlive() then
			return
		end
