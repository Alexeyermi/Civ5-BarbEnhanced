...
function InitMinorCivList()
	...
	for iPlayerLoop = GameDefines.MAX_MAJOR_CIVS, GameDefines.MAX_CIV_PLAYERS-1, 1 do
		...
		if( pOtherPlayer:IsMinorCiv() and 
			...
			-- Fix display name for barbarians
			if iOtherTeam == GameDefines.MAX_CIV_PLAYERS then
				local primaryColor, _ = pOtherPlayer:GetPlayerColors();
				...
				--controlTable.MinorName:SetText( Locale.ConvertTextKey(GameInfo.MinorCivPirates[tBarbarians[iPlayerLoop].ID].Description) )
				controlTable.MinorName:SetText( "Barbarian player ".. iPlayerLoop )
				controlTable.MinorName:SetColor( color, 0 );
				controlTable.StatusText:SetText( Locale.ConvertTextKey( "TXT_KEY_DIPLO_MAJOR_CIV_DIPLO_STATE_WAR" ) );
			else
			-- End add
