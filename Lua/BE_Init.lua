...
SPAWN_COOLDOWN = 10		--minimum turns between spawns
ATTACK_COOLDOWN = 20	--minimum turns between city attack
ATTACK_MELEE = 5		--melee attackers
ATTACK_RANGED = 3		--ranged attackers
ATTACK_SIEGE = 2		--siege attackers
ATTACK_PCT = 15			--percent chance barb will attack each turn
...
if #tMinorPlayers > 0 then
	...
	for i, iPlayer in ipairs(tOpenMinorPlayers) do
		...
		local player = Players[iPlayer]
		iBarbarianTeam = GameDefines.MAX_CIV_PLAYERS
		PreGame.SetTeam(iPlayer, iBarbarianTeam)
		for iEnemy = 0, GameDefines.MAX_MAJOR_CIVS-1 do
			local enemy = Players[iEnemy]
			local iEnemyTeam = enemy:GetTeam()
			local enemyTeam = Teams[iEnemyTeam]
			enemyTeam:DeclareWar(iTeam)
			--enemyTeam:SetPermanentWarPeace(iTeam)						--permanent war doesn't work for minors
			--Teams[iBarbarianTeam]:SetPermanentWarPeace(iEnemyTeam)
		end
